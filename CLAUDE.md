...
算法：衰减加权否决模型 + EV双重条件 + 亚盘六层信号（S0 CLV方向）+ 双重确认决策。
...
### CLV追踪规格（2026-03-29更新）
**方式一：方向验证（主要指标）**
- 补录字段：`betSide`（下注方向）+ `pinnDirection`（平博关盘赔率最低项方向）
- 系统判断：一致→"✓ 跟上聪明钱"，不一致→"✗ 与聪明钱相反"
- 统计：一致场次数/占比，长期>50%说明分析有价值

**方式二：体彩内部CLV（可选）**
- 补录字段：`tcBuy`（买入时体彩赔率）+ `tcClose`（体彩关盘同选项赔率）
- 公式：`tc_clv = tcBuy / tcClose`（>1=买到更好价格）
- **注：跨市场比较（体彩买入 vs 平博关盘）无意义，已废弃**
